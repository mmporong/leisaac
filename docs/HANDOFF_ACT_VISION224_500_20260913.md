...
> 최신 원인 조사 및 다음 AI 전달용 문서:
> [초기 카메라 관측 불일치와 ACT 집기 실패 인계](HANDOFF_MIMIC_ACT_ROOT_CAUSE_20260921.md).
> 초기 영상 결함은 확인했지만 수정 후 집기 성공 개선은 아직 검증하지 않았다.

## 2026-09-21 2,000 → 10,000스텝 이어 학습

사용자 요청으로 시작 관절 상태를 확인하고 기존 체크포인트에서 재개했다.
학습 61개·검증 15개 모두 첫 관절 상태가 `[0,0,0,0,0,0]`으로 평가 시작 관절 상태와
일치했다. 관절 시작 자세 불일치는 없지만, 이것만으로 영상·물체 배치 분포가 같다고
단정하지 않는다. 2,000스텝 모델과 데이터를 보존한다.

- `--resume=true`와 2,000스텝 `train_config.json`을 사용한다. 총 목표는 10,000이며
  추가 학습량은 8,000스텝이다. 새로운 옵티마이저를 쓰는 fine-tuning이 아니다.
- 같은 61/15 분리, batch 8, 상태·행동 정규화와 ACT 구조를 유지한다.
  로그에서 데이터 순서 `epoch 0, sample 16000` 재개를 확인했다.
- 10,000스텝 마지막 모델로 오프라인 검증과 seed 4100/4101/4102 비교 평가를 수행한다.
  성공 판정·horizon 1,200·매 평가 새 프로세스·영상 규격은 2,000스텝 실험과 같다.
  같은 seed의 전후 비교이며 새 미학습 조건에 대한 일반화 평가로 주장하지 않는다.
- 새 출력 경로는 `outputs/act_resume_10000_20260921/`다. strict production gate나
  실물 실행 권한은 변경하지 않았다. 별도 프로젝트 GPU 프로세스는 건드리지 않는다.

학습 61개 raw 초기 큐브 위치를 다시 매핑한 결과 X 범위는 0.279092~0.420714 m,
Y 범위는 -0.436602~-0.361076 m다. seed 4100은 X=0.275244로 범위 밖,
4102는 Y=-0.342487로 범위 밖이며 4101만 축별 bounding box 안이다.
500개 원본 전체 범위를 학습 61개 범위로 혼동하지 않는다.
축별 범위 안이라는 조건도 결합분포의 충분한 학습 지원을 보장하지 않는다.
기존 3개 seed는 변경하지 않아 전후 비교를 보존하되 범위 안/밖을 구분해 해석한다.

총 10,000스텝 학습은 exit 0으로 완료했고, 체크포인트 파라미터 유한성과 기존 학습 집합
정규화 통계 일치 검증을 통과했다. 동일 검증 데이터의 균등 500프레임 RMSE는
0.088081 → 0.050011 rad, 15개 시작 프레임 RMSE는 0.293607 → 0.145189 rad로 줄었다.
오프라인 오차 감소와 실제 집기 성공은 구분한다.
...
| 같은 seed 3개 집기 성공 | 0/3 | 0/3 |
| 최대 큐브 상승 | 모두 0 m | 모두 0 m |
| 관절 제한에 의한 명령 보정 | 모두 0스텝 | 4100/4101: 0, 4102: 290스텝 |

학습과 평가 프로세스 모두 exit 0으로 완료했다. 3개 평가 영상은 각 1280×480·60fps·
1,200프레임이다. 전후 초기 물리 상태 해시는 같지만 영상은 비트 단위로 같지 않다.
초기 영상 픽셀 MAE(0~255)는 앞 카메라 0.069~0.131, 손목 0.017~0.033이다.
같은 평가 설정의 비교이지 완전히 동일한 렌더 관측으로 통제한 실험이라고 주장하지 않는다.

10,000스텝 seed 4102는 그리퍼 목표가 관절 제한을 벗어나 290스텝에서 보정됐으며
최대 보정량은 0.015540 rad다. 나머지 두 seed는 보정 0스텝이다.
보정이 세 실패의 공통 원인이라고 단정할 수는 없지만, 그리퍼 출력 진단 대상에 포함한다.

결론: 추가 학습으로 보류 데이터의 목표각 예측은 개선됐지만 실제 집기는 개선되지 않았다.
seed 4101은 학습 위치 bounding box 안에서도 실패했으므로 두 OOD 위치만으로 모든 실패를
설명할 수 없다. 반대로 3회 실패만으로 원본 시연 부족이나 Mimic 무효를 단정하지 않는다.
다음은 학습 범위 안의 시작 장면에서 첫 접근·그리퍼 닫힘까지 정책 궤적과 성공 시연을
비교하는 것이다. 학습량 추가 확대나 리더 재촬영을 자동으로 진행하지 않는다.

모델·원래 2,000스텝 체크포인트·비교 영상·로그는 모두 보존했다.
[이어 학습 측정 근거](evidence/act_resume_10000_20260921.json)에 재개 명령·출처 해시·
학습 위치 범위·전후 평가와 영상 해시를 기록했다.

```bash
cd "/data/$USER/leisaac"
"$HOME/miniforge3/envs/lerobot/bin/python" -u -m lerobot.scripts.lerobot_train \
  --config_path="/data/$USER/leisaac/outputs/act_pilot_76_20260921/model/checkpoints/002000/pretrained_model/train_config.json" \
  --resume=true --steps=10000 --save_freq=10000 \
  --output_dir="/data/$USER/leisaac/outputs/act_resume_10000_20260921/model" \
