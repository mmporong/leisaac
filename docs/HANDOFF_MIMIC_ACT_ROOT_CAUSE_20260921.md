...
**남은 원인 3건의 합의 계획과 세션 이관 정리는 `docs/HANDOFF_REMAINING_CAUSES_20260922.md`에 있다.** 다음 작업은 그 문서부터 읽는다.
...
### 13.8 초기 영상만 고친 데이터로 학습한 결과 (한 요소 변경 실험)

근거: `docs/evidence/reset_fixed_dataset_experiment_20260921.json`. 용량 정리 기록: `docs/evidence/storage_cleanup_20260921.json`.

`scripts/mimic/rerender_initial_frames.py`로 76개 후보의 frame 0·1만 저장된 `initial_state`에서 재렌더한 파생 raw를 만들었다
(손목 MAE new0 vs old2 중앙값 3.98, 관절 상태 오차 0). 같은 감사 게이트로 76개가 그대로 선택됐고,
변환 결과의 에피소드·프레임 수와 `info.json`, seed 43 분리의 에피소드 인덱스가 원본과 동일했다. 즉 두 데이터셋의 차이는 에피소드당 두 프레임뿐이다.
같은 10,000스텝 레시피로 고정 데이터와 원본 데이터(대조군)를 각각 새로 학습하고 seed 4100~4102로 평가했다.

| 항목 | 기존 이어 학습 10k | 대조군 새 10k (원본) | 고정 데이터 새 10k |
| --- | --- | --- | --- |
| 오프라인 첫 프레임 RMSE (rad) | 0.1452 | 0.1452 | 0.1063 |
| 오프라인 균등 500 RMSE (rad) | 0.0500 | 0.0500 | 0.0497 |
| 집기 성공 | 0/3 | 0/3 | 0/3 |
| 관절 제한 보정 스텝 (4100/4101/4102) | 0/0/290 | 0/0/267 | 0/0/0 |

대조군은 기존 이어 학습과 오프라인 지표가 비트 단위로 같아 학습이 결정적이고, 이어 학습과 새 학습이 동등함을 확인했다.

같은 demo_9 초기 장면에서 청크 30으로 비교:

| 항목 | 시연 | 기존 모델 | 고정 데이터 모델 |
| --- | --- | --- | --- |
| 명령 RMSE vs target, 60 / 240 스텝 | - | 0.167 / 0.498 | 0.119 / 0.118 |
| 그리퍼 0.5 rad 미만 첫 스텝 | 278 | 138 | 없음 (최소 0.75) |
| 큐브 XY 5 mm 이동 첫 스텝 | 279 | 184 | 없음 |
| 결과 | 성공 | 큐브 밀고 실패 | 접촉 없이 실패 |

확정: 초기 영상 수정은 시작 프레임 예측 오차를 27% 줄이고 동일 장면에서 접근 궤적 추종을 크게 개선하지만, 집기 성공은 0/3 그대로다.
고정 데이터 모델은 접근은 시연과 비슷하게 하면서 그리퍼를 닫지 않는다. 초기 영상 결함은 원인 중 하나였지 단독 원인이 아니다.

추정(검증 안 됨): 남은 실패는 도달 정확도보다 파지 결정(그리퍼 닫힘 시점) 쪽 문제다. 시연 1개·seed 3개 근거이므로 성공률 주장은 아니다.

다음 후보: (1) 동일 장면에서 고정 모델의 그리퍼 명령 시계열과 시연의 닫힘 구간 입력(손목 영상·관절)을 대조해 닫힘 신호가 학습됐는지 확인,
...
### 14.7 정책 PNG 확보 후 재현성 검사

2026-09-22 후속 실행에서 PNG 저장 기능과 폐루프 프로브를 구현했고, 240~340스텝 정책 입력 PNG 202장을 확보했다.
그러나 초기 물리 상태 SHA가 같아도 이전 실행과 초기 RGB가 조금 달라 첫 명령부터 분기했다. PNG 저장을 끈 대조 실행도 같은 문제를 보였다.
세 실행의 초기 입력을 CPU로 다시 넣으면 각각의 첫 30개 저장 명령을 비트 단위로 재현한다. 따라서 이 구간은 모델 계산의 비결정성이 아니라 입력 RGB 차이로 설명된다.
어떤 렌더 설정·히스토리·난수 요인이 RGB 차이를 만드는지, 이 차이가 집기 실패를 설명하는지는 아직 미결이다.

새 trace의 close 잔존 표본은 **14/30**으로 계획 하한 15개보다 적다. 기존 17개와 섞지 않는다.
strict 재현 검사도 실패해 **1.2 완료 조건과 1.1.7 폐루프 원인 판정은 통과하지 못했다**. 프로브는 판정 결과를 만들기 전에 차단됐고 추가 학습은 하지 않았다.
전체 테스트 189개 PASS, 원본/파생 raw 81개 크기·mtime과 수정 모델 SHA 보존을 확인했다.

다음 실행자는 [후속 인계 12절](HANDOFF_REMAINING_CAUSES_20260922.md#12-정책-영상-수집과-재현-실패-분리-2026-09-22-후속)과
[재현 진단 근거](evidence/closed_loop_reproduction_20260922.json)를 읽고 초기 RGB 재현성 조사부터 이어간다.

### 14.8 팔·그리퍼 명령 분리와 Diffusion 대안 준비

사용자의 후속 요청으로 명령 채널별 분리 실험 runner를 구현·검증했다. 같은 evaluator에 시연 팔/그리퍼 명령을 선택적으로 넣고,
각 seed의 전체 시연 명령 대조가 성공해야 다른 조건으로 넘어간다. 기존 시연 재생의 642스텝 성공은 이미 확인된 물리 제어 근거이며 새 ACT 성공이 아니다.
다른 프로젝트가 GPU를 사용해 이 새 진단은 아직 실행하지 않았다.

ACT 외 대안으로 Diffusion Policy의 실제 수정 76개 데이터 연결을 CPU에서 검사했다. 라벨 정렬, 정규화, forward/backward, 추론 출력 검사는 통과했다.
소형 무작위 초기 모델이며 optimizer 업데이트는 없으므로 학습·집기 성공·GPU 메모리 적합성의 근거는 아니다.
다음 작업은 [후속 인계 13·14절](HANDOFF_REMAINING_CAUSES_20260922.md#14-다음-세션에서-이어갈-순서)에 정리했다. CPU 완료 단계를 다시 실행하지 않는다.
