...
1. **Mimic 생성기는 소스 시연 10개를 전부 썼다.** 76개 후보가 6계열로 줄어든 것은 감사 게이트의 **관절 클리핑 필터**(주로 `wrist_flex`) 때문이다. 품질 게이트를 통과하고도 클리핑만으로 빠진 에피소드가 110개다. 재생성 없이 기존 raw 500개에서 허용오차 0.05 풀 188개(9계열)를 만들 수 있다.
...
**디스크 없이도 시작할 수 있는 단계**: 2.0(CPU, 2 MB)과 1.1(CPU, 약 15 MB)은 지금 바로 가능하다. 1.1.7도 기존 trace로 가능하다.
